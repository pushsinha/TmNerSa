...
# -*- coding: utf-8 -*-
"""
Created on Fri Mar 30 17:14:19 2018
...
data = data.na.drop(subset = ["article_text"])
...
def cleanup_text(text):
    ...
    text_out = [word for word in text_out if len(word)>2 and word not in stopwords]     # Remove stopwords and words under X length
    text_out = " ".join(lemma.lemmatize(word,'v') for word in text_out)
    
    return text_out

udf_cleantext = udf(cleanup_text, StringType())
clean_text = data.withColumn("clean_comm", udf_cleantext(data.article_text))
f = clean_text.select("clean_comm")
import numpy as np
np.savetxt("cleaned_articles.txt", f.toPandas().values, fmt = '%s')
